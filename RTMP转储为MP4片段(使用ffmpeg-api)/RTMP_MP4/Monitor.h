...
public:
	...
	static std::queue<VideoInfo> video_info_list;//共享存储队列
...
public:
	...
	bool get_online_state();//更新设备的在线信息
	void get_input_url();//从数据库更新rtmp地址

	int open_input();//打开输入流
	void close_input();//关闭输入流
	int open_output(char* output_url);//打开输出流并创建MP4文件
	void close_output();//写文件尾并关闭输出流
	int write_frame();//写一帧数据
	void write_trailer();//写文件尾
